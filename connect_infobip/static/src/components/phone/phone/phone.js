...
import {Calls} from "@connect_infobip/components/phone/calls/calls"
...
// Thin adapter exposing a Twilio-Device-like session API on top of an
...
class InfobipSession {
    constructor(call) {
        ...
        let customData = {}
        try {
            customData = (typeof call.customData === 'function'
                ? call.customData() : call.customData) || {}
        } catch (e) {
            console.warn(e)
        }
        ...
        let from = ''
        try {
            from = (typeof call.from === 'function' ? call.from() : call.from) || ''
        } catch (e) {
            from = ''
        }
        this.parameters = {From: from}
        call.on('established', (event) => {
            this._attachStream(event)
            if (!this.accepted) {
                this.accepted = true
                this._emit('accept', event)
            }
        })
        call.on('early-media', (event) => this._attachStream(event))
        call.on('hangup', (event) => {
            if (!this.ended) {
                this.ended = true
                this._emit(this.accepted ? 'disconnect' : 'cancel', event)
            }
        })
        call.on('error', (event) => {
            console.warn('Infobip call error:', event)
            if (!this.ended) {
                this.ended = true
                this._emit(this.accepted ? 'disconnect' : 'cancel', event)
            }
    ...
    _attachStream(event) {
        const audio = document.getElementById('connect-infobip-remote-audio')
        if (audio && event && event.stream) {
    ...
    reject() {
        if (typeof this.call.decline === 'function') {
            this.call.decline()
        } else {
            this.call.hangup()
        }
    ...
    sendDTMF(sequence) {
        this.call.sendDTMF(sequence)
    }

    refer(destination) {
        console.warn('Transfer is not supported by the Infobip web phone yet.', destination)
    }
}

// The vendored UMD bundle may expose the SDK either as an InfobipRTC
// namespace object or as plain globals (CDN build).
function getInfobipRtcApi() {
    const ns = window.InfobipRTC && window.InfobipRTC.createInfobipRtc
        ? window.InfobipRTC : window
    return {
        createInfobipRtc: ns.createInfobipRtc,
        ApplicationCallOptions: ns.ApplicationCallOptions,
    }
}
...
export class Phone extends Component {
    ...
    constructor() {
        ...
        this.callsConfigId = this.props.token_data.calls_config_id
        this.viaRest = this.props.token_data.via_rest
        this.tokenExpiration = this.props.token_data.expiration
        this.callStatus = {
            NoAnswer: 'noanswer',
            Busy: 'busy',
            Rejected: 'busy',
    ...
    setup() {
        ...
        onWillStart(async () => {
            await loadJS('/connect_infobip/static/src/lib/infobip.rtc.js')
    ...
    async updateToken() {
        // The infobip-rtc client has no in-place token update: fetch a
        // fresh token and re-initialize the client.
        const now = (new Date()).getTime()
        if (now - this.lastTokenRefresh < 10000) {
            return
        }
        this.lastTokenRefresh = now
        const token_data = await this.orm.call('connect.user', 'get_infobip_client_token')
        if (token_data.token) {
            this.token = token_data.token
            this.tokenExpiration = token_data.expiration
            try {
    ...
    _scheduleTokenRefresh() {
        // Infobip tokens hard-expire: refresh proactively at ~90% TTL.
        ...
        const ttl = Date.parse(this.tokenExpiration) - Date.now()
        if (isNaN(ttl) || ttl <= 0) {
            return
        }
    ...
    initUserAgent() {
        ...
        if (!api.createInfobipRtc) {
            console.error('infobip-rtc SDK is not loaded')
            return
        }
        self.userAgent = api.createInfobipRtc(self.token, {debug: false})

        self.userAgent.on('connected', () => {
            self.sipRegistered = true
        })

        self.userAgent.on('disconnected', (event) => {
            console.log('Infobip RTC disconnected:', event)
            self.sipRegistered = false
            self.updateToken().then()
        })

        self.userAgent.on('reconnecting', () => {
            self.sipRegistered = false
        })

        self.userAgent.on('reconnected', () => {
            self.sipRegistered = true
        })
        ...
        const onIncoming = (event) => {
            ...
            if (self.session === null) {
                self._handleIncoming(incomingCall).then()
            } else if (typeof incomingCall.decline === 'function') {
                incomingCall.decline()
            }
        }
        self.userAgent.on('incoming-application-call', onIncoming)
        self.userAgent.on('incoming-webrtc-call', onIncoming)
    ...
    _playRingtone() {
        ...
        if (!this.ringtoneAudio) {
            this.ringtoneAudio = new Audio('/connect_infobip/static/src/sounds/incoming-call.mp3')
    ...
    async _handleIncoming(call) {
        const self = this
        const session = new InfobipSession(call)
    ...
    async makeCall(props) {
        ...
        if (self.viaRest || !self.callsConfigId || !api.ApplicationCallOptions
            ...
            try {
                await self.orm.call('connect.settings', 'originate_call',
                    [phoneNumber], {auto_answer: true})
            } catch (e) {
                console.error(e)
                self.state.phone_status = self.status.ended
                await self.endCall()
        ...
        const options = api.ApplicationCallOptions.builder()
            ...
            .build()
        const call = self.userAgent.callApplication(self.callsConfigId, options)
        self.session = new InfobipSession(call)
