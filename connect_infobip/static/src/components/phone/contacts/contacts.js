...
import {setFocus} from "@connect_infobip/js/utils"
...
export class Contacts extends Component {
    static template = 'connect_infobip.contacts'
    static props = {
        bus: Object,
    ...
    searchUser() {
        ...
        if (self.searchQuery) {
            self.orm.searchRead(
                "connect.user",
                [
                    '|', ['infobip_exten_number', '=ilike', `%${self.searchQuery}%`],
                    ['user', '=ilike', `%${self.searchQuery}%`]
                ],
                ['id', 'name', 'infobip_exten_number', 'user'],
                {order: 'infobip_exten_number asc', limit: 10}
            ).then((records) => {
                // Templates render user.exten_number; expose the provider field under the legacy key.
                records.forEach((r) => { r.exten_number = r.infobip_exten_number })
