...
class Exten(models.Model):
    _name = 'connect.telnyx.exten'
    _description = 'Telnyx Extension'
    _order = 'number'
    ...
    def _render_dst(self, request=None, params=None):
        # connect.user is a shared ledger model, so its Telnyx render
        # method carries the telnyx_ prefix (connect_twilio owns render()).
        self.ensure_one()
        if self.dst._name == 'connect.user':
            return self.dst.telnyx_render(request=request, params=params)
        return self.dst.render(request=request, params=params)

    def _get_dialplan(self):
        for rec in self:
            try:
                result = rec._render_dst({})
    ...
    def render(self, request=None, params=None):
        ...
        # Copy before adding keys: a shared default {} would leak
        # ExtenID/ExtenNumber across calls.
        params = dict(params or {})
        params['ExtenID'] = self.id
        params['ExtenNumber'] = self.number
        return self._render_dst(request=request, params=params)
