...
When an outbound web-phone call fails before it is answered, the Telnyx
phone widget surfaces the reason as an Odoo notification instead of
silently returning to the keypad: busy, rejected, number not found, or a
generic *"Call failed"* with the underlying cause/SIP code.

!!! warning "Billing-blocked account"
    If the Telnyx account balance is exhausted, Telnyx rejects **every**
    origination with `404 / UNALLOCATED_NUMBER` *before* any TeXML
    routing — no webhook or CDR is produced. To disambiguate this from a
    wrong number, the widget calls `connect.settings.telnyx_check_call_failure()`,
    which fetches `GET /v2/balance` server-side (`sudo`) and, when the
    available credit is `<= 0`, shows a sticky red notification naming
    the real cause. Members of **Connect Administrator**
    (`connect.group_admin`) also see the current balance in the message;
    plain users get the message without amounts. **Top up the Telnyx
    account** to restore calling.

    The method is callable by `connect.group_user` but only ever returns
    a boolean plus a message — never the API key or raw Telnyx payloads.
