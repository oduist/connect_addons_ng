    ...
    searchUser() {
        ...
        if (self.searchQuery) {
            self.orm.searchRead(
                "connect.user",
                [
                    '|', ['telnyx_exten_number', '=ilike', `%${self.searchQuery}%`],
                    ['user', '=ilike', `%${self.searchQuery}%`]
                ],
                ['id', 'name', 'telnyx_exten_number', 'user'],
                {order: 'telnyx_exten_number asc', limit: 10}
            ).then((records) => {
                // Templates render user.exten_number; expose the provider field under the legacy key.
                records.forEach((r) => { r.exten_number = r.telnyx_exten_number })
