...
- New method `freeswitch_api(command, args)` on `connect.settings` — catches all exceptions and logs errors without raising, so gateway CRUD is never blocked by FS connectivity issues. (ADR-028 later split the connection logic into a `_freeswitch_rpc()` helper returning a `(result, error)` tuple so the Server Status field can distinguish the failure mode; `freeswitch_api()` keeps the same `response | False` contract.)
