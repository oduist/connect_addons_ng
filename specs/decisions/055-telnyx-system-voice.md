...
2. Store the voice as a dynamic Selection. Cache the account catalog returned
   by `GET /v2/text-to-speech/voices`, refresh it during the normal Telnyx
   account sync, and provide a dedicated refresh button. Keep the basic TeXML
...
3. Before a TeXML response leaves the module, parse it and add the system
...
4. Replace the hardcoded Telnyx user-prompt fallback with `System Voice`.
   Make the callflow voice optional and use `System Voice` when no callflow
   override is configured.
5. Treat catalog refresh as optional during the full account sync: a catalog
...
- A free-form Char would accept the complete Telnyx contract but would not
  give administrators the requested list of voices available to their
  account.
...
- The settings form offers the catalog known to the Telnyx account without a
