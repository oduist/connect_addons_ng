...
Talks to /3cx/webhook/* and /3cx/api/* on Odoo, authenticated with the
shared ``threecx_api_key`` carried as ``Authorization: Bearer <token>``.
Events are batched (size/time window) into single POSTs; when Odoo is
down the queue grows in memory and the reconciler heals whatever
overflows.
"""
...
class OdooClient:
    ...
    def enqueue_event(self, event: dict) -> None:
        """Queue a participant event for delivery. Drops the oldest when
        full — the reconciliation loop emits synthetic removes for
        anything the webhook never saw."""
        try:
    ...
    async def outbox_worker(self) -> None:
        ...
        while True:
            ...
            while True:
                try:
                    await self.post("/3cx/webhook/events", batch)
