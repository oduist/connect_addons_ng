...
class AsteriskTemplate(models.Model):
    """Jinja2 snippets of Asterisk configuration rendered for the customer.

    The target market is existing Asterisk installations (FreePBX, Issabel,
    plain Asterisk) — the module never pushes configuration; it renders
    copy-paste / include snippets (pjsip wizard peers, manager.conf user)
    that the administrator applies on the PBX.
    """
    _name = 'connect.asterisk.template'
    _description = 'Asterisk Config Template'
    _order = 'key'
    ...
    def render(self, key, context):
        ...
        if content is None:
            raise ValueError("Asterisk template '{}' not found".format(key))
