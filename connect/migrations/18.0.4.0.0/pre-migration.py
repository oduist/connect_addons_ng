"""Pre-migration: provider model separation (18.0.4.0.0, ADR-031).
...
connect.asterisk.*). This script only protects the data:
  ...
  2. Delete `ir.model.fields.selection` xmlid rows owned by the connect
     modules — same Odoo 18 ondelete-str bug dodge as in 18.0.3.1.0:
...
def migrate(cr, version):
    ...
    _logger.info("connect pre-migration: from %s to 18.0.4.0.0", version)

    # 1. Archive the moving tables.
    for source, archive in ARCHIVE_TABLES:
        _archive_table(cr, source, archive)

    # 2. Odoo 18 ondelete-str bug dodge (see module docstring). Provider
