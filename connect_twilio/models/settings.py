"""Twilio extensions to `connect.settings` (ADR-031 / ODU-46).

Twilio-specific fields and methods live back on the shared
`connect.settings` model (the pre-ODU-22 design), reached as install-
conditional notebook tabs on the single General Settings form. Field
names carry the `twilio_` prefix to avoid colliding with the other
providers sharing this model (`account_sid`, `auth_token`,
`display_auth_token`, `fetch_call_prices` were never prefixed and keep
their names).

This module also keeps the module-level constants and helper functions
imported by other Twilio files, and registers the module for licensing.
"""
# -*- coding: utf-8 -*-
import logging
import re
...
PROTECTED_FIELDS = {'display_auth_token', 'display_twilio_api_secret'}
...
class Settings(models.Model):
    ...
    def get_client(self):
        try:
            ...
            if cfg_su.twilio_region:
                client.region = cfg_su.twilio_region
            if cfg_su.twilio_edge:
                client.edge = cfg_su.twilio_edge
        ...
        try:
            ...
            try:
                ...
                if ('20404' in str(balance_error)
                    ...
                    self.sudo().twilio_balance = balance
                    self.env['connect.settings'].connect_notify(
