    ...
    searchUser() {
        ...
        if (self.searchQuery) {
            self.orm.searchRead(
                "connect.user",
                [
                    '|', ['twilio_exten_number', '=ilike', `%${self.searchQuery}%`],
                    ['user', '=ilike', `%${self.searchQuery}%`]
                ],
                ['id', 'name', 'twilio_exten_number', 'user'],
                {order: 'twilio_exten_number asc', limit: 10}
            ).then((records) => {
                // Templates render user.exten_number; expose the provider field under the legacy key.
                records.forEach((r) => { r.exten_number = r.twilio_exten_number })
