...
class ElevenlabsAgent(models.Model):
    ...
    def create_extension(self):
        self.ensure_one()
        return self.env["connect.twilio.exten"].create_extension(self, "connect.elevenlabs_agent")
    ...
    def _resolve_caller_id(self, request):
        ...
        if caller.startswith("+") and caller[1:].isdigit():
            return caller
        default = self.env["connect.twilio.outgoing_callerid"].sudo().search(
    ...
    def build_initiation_payload(self, caller="", called="", agent_uid="", call_sid="", call_ref=""):
        ...
        dyn["users_directory"] = ", ".join(
            "{} <{}>".format(u.user.name, u.twilio_exten.number)
            for u in users if u.user and u.twilio_exten
        )
        ...
        published_extens = self.env["connect.twilio.exten"].sudo().search(
            [("is_published", "=", True)])
        if published_extens:
