...
2. Open **FreeSWITCH → Configuration → Settings**, page **Firewall**, as an admin:
   ...
   * Adjust port lists and timeouts if you need to deviate from the
     defaults.
3. FreeSWITCH → Firewall → **Whitelist**: add your trunk providers,
...
4. FreeSWITCH → Firewall → **Agent Status**: should show the agent
...
* **From Odoo** — open `FreeSWITCH → Firewall → Events`. Every row
  ...
  longer banned.
* **Permanent block** — `FreeSWITCH → Firewall → Blacklist`, add an
