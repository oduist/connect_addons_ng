...
## Softphone Localization

The Verto browser softphone (dialer, parking panel, endpoint-password
widget and parking toasts) is translated and follows each user's **Odoo
interface language**. The module ships a hand-maintained catalog for
**German, French, Italian and Russian**; any other language falls back to
English.

Notes for administrators:

- Translation files use **base language codes** (`i18n/de.po`, `fr.po`,
  `it.po`, `ru.po`). Odoo loads `i18n/<base>.po` before `i18n/<ll_CC>.po`,
  so a single `de.po` serves both `de_DE` and `de_CH`, `fr.po` serves
  `fr_FR`/`fr_CH`, and `it.po` serves `it_IT`/`it_CH`.
- The catalog is scoped to the softphone UI, not the whole module —
  field and view labels follow Odoo's standard translation flow.

!!! warning "Restart after editing translations"
    JS/OWL translations are read straight from the `.po` files (there is
    no database import step), but Odoo keeps an in-process translation
    cache. After changing a `.po` file on a live deployment you must
    **restart the Odoo server** for the new wording to take effect.
